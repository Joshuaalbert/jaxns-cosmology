...
def main(best_posteriors_dir, hyper_param_search_dir, problems_list: List[str], ns_list: List[str],
    ...
    with open("all_results.txt", "w") as f:
        f.write("#problem likelihood_evals ns run_dir wasserstein_lower wasserstein_upper\n")
    fig, axs = plt.subplots(len(problems_list), 1, figsize=(10, 12), squeeze=False, sharex=True)
    ...
    for j, problem in enumerate(problems_list):
        ...
        axs[j][0].axhline(rule_of_thumb, color='black', linestyle='--', label='Rule of thumb')
        # axs[j][0].set_title(f"{problem} Wasserstein Upper Bound")
        # Title as inset upper right of plot, right aligned
        axs[j][0].text(0.9, 0.9, f"{problem} Wasserstein Upper Bound", ha='right', va='top', transform=axs[j][0].transAxes)

        axs[j][0].set_ylabel('Wasserstein distance')
        # manually set legend, with colors, remove duplicate labels
        handles, labels = axs[j][0].get_legend_handles_labels()
        by_label = dict(zip(labels, handles))
        # axs[j][0].legend(by_label.values(), by_label.keys(), loc='upper left')
        # Two column legend, mostly transparent
        axs[j][0].legend(by_label.values(), by_label.keys(), loc='upper left', ncol=2, framealpha=0.5)
        # log scale x axis
        axs[j][0].set_xscale('log')
    axs[-1][0].set_xlabel('Number of likelihood evaluations')
    fig.tight_layout()
    fig.subplots_adjust(hspace=0.)
    plt.savefig(f'plots/wasserstein_upper.png')
    plt.show()
